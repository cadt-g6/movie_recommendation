...
class Top10MoviesAnalystic:
    def __init__(self) -> None:
        self.movie_df = pd.read_csv(
            DatasetType.movies_metadata.sample_path())

    def top_10_based_on_popularity(self):
        top_popular_movies = self.movie_df.sort_values(
            by=['popularity'], ascending=False)
        return top_popular_movies.head(10)

    def top_10_based_on_rating(self):
        top_avg_quan_movies = self.movie_df.sort_values(
            by=['avg_rating', 'quantity'], ascending=False)
        return top_avg_quan_movies.head(10)

    def top_10_based_on_revenue(self):
        top_revenue_movies = self.movie_df.sort_values(
            by=['revenue'], ascending=False)

        return top_revenue_movies.head(10)

    def top_10_based_on_profit(self):
        cleaned_budget_df = self.movie_df[self.movie_df["budget"] > 5000]
